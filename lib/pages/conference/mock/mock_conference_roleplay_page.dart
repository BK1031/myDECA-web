...
class MockConferenceRoleplayPage extends StatefulWidget {
  ...
  @override
  _MockConferenceRoleplayPageState createState() =>
      _MockConferenceRoleplayPageState(this.id);
}

class _MockConferenceRoleplayPageState
    extends State<MockConferenceRoleplayPage> {
      ...
      if (stopwatch.isRunning) {
        setState(() {
          if (DateTime.now().isAfter(endPrepTime)) {
            ...
            stopwatch.stop();
            alert(
                "Your preparation period has ended! Please join the judging room for your scheduled judging period.");
          }
        });
      }
    });
    ...
    showDialog(
        ...
        builder: (context) => AlertDialog(
              ...
              content: Container(
                  width: 500,
                  child: new Text(
                      "It looks like you still have some time before your scheduled judging time. Are you sure you want join now?",
                      style: TextStyle(color: currTextColor))),
              actions: [
                new FlatButton(
                    child: new Text("CANCEL"),
  ...
  void createBreakdown(String event) {
    ...
    setState(() {
      breakdownWidgets.add(new Text(
        "Written Evaluation:",
        style: TextStyle(fontSize: 20, color: mainColor),
      ));
      breakdownWidgets.add(new Padding(
    ...
    for (int i = 0; i < roleplayRubrics[event][0].length; i++) {
      ...
      setState(() {
        breakdownWidgets.add(new Text(
          "${roleplayRubrics[event][0][i]}:  ${savedScores[0][i]}",
    ...
    for (int i = 0; i < roleplayRubrics[event][1].length; i++) {
      ...
      setState(() {
        breakdownWidgets.add(new Text(
          "${roleplayRubrics[event][1][i]}:  ${savedScores[1][i]}",
          style: TextStyle(fontSize: 17),
        ));
      });
    }
                                    ...
                                    children: [
                                        ...
                                        children: [
                                          Column(
                                            ...
                                            children: [
                                              ...
                                              new Text(
                                                  "Preparation Time: ${DateFormat("MMMd").format(startTime)} (${DateFormat("jm").format(startTime.subtract(Duration(minutes: 15)))} - ${DateFormat("jm").format(startTime.subtract(Duration(minutes: 5)))})",
                                                  style: TextStyle(
                                                      fontSize: 20,
                                                      color: mainColor)),
                                              new Text(
                                                  "Presentation Time: ${DateFormat("MMMd").format(startTime)} (${DateFormat("jm").format(startTime)} - ${DateFormat("jm").format(startTime.add(Duration(minutes: 10)))})",
                                                  style: TextStyle(
                                                      fontSize: 20,
                                                      color: mainColor)),
                                            ],
                                      ...
                                      new Visibility(
                                          ...
                                          child: new SelectableText(
                                            "Team ID: ${roleplayTeamID} [${selectedRoleplay}] ${currUser.firstName} ${currUser.lastName}, Judge: ${judge.firstName} ${judge.lastName} @ ${DateFormat("jm").format(startTime)}",
                                            style: TextStyle(
                                      ...
                                      new Visibility(
                                        visible:
                                            score == -1 && stopwatch.isRunning,
                                        child: Container(
                                          padding: EdgeInsets.all(8),
                                          child: new Center(
                                            child: new Text(
                                              "Time Remaining: ${stopwatch.isRunning ? _printDuration(endPrepTime.difference(DateTime.now())) : ""}",
                                              style: TextStyle(fontSize: 17),
                                            ),
                                          ),
                                        ),
                                      ...
                                      new Visibility(
                                        ...
                                        child: new FlatButton(
                                          child: new Text(
                                              "Can't see the prompt? Click here!"),
