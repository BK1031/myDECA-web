...
class MockConferenceWrittenPage extends StatefulWidget {
  ...
  @override
  _MockConferenceWrittenPageState createState() =>
      _MockConferenceWrittenPageState(this.id);
}
...
class _MockConferenceWrittenPageState extends State<MockConferenceWrittenPage> {
            ...
            fb
              ...
              if (value.snapshot.val()["written"] != null) {
                ...
                mockConferenceEvents.forEach((key, value) {
                  if (value.contains(selectedWritten))
  ...
  void createBreakdown(String event) {
    ...
    for (int i = 0; i < writtenRubrics[event][0].length; i++) {
      ...
      setState(() {
        breakdownWidgets.add(new Text(
          "${writtenRubrics[event][0][i]}:  ${savedScores[0][i]}",
    ...
    for (int i = 0; i < writtenRubrics[event][1].length; i++) {
      ...
      setState(() {
        breakdownWidgets.add(new Text(
          "${writtenRubrics[event][1][i]}:  ${savedScores[1][i]}",
          style: TextStyle(fontSize: 17),
        ));
      });
    }
                                    ...
                                    children: [
                                      ...
                                      new Visibility(
                                          ...
                                          child: new SelectableText(
                                            "Team ID: ${writtenTeamID} [${selectedWritten}] ${currUser.firstName} ${currUser.lastName}, Judge: ${judge.firstName} ${judge.lastName} @ ${DateFormat("jm").format(startTime)}",
                                            style: TextStyle(
